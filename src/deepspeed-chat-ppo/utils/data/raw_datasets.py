    ...
    def __init__(self, output_path, seed, local_rank, dataset_name):
        ...
        if os.path.exists(dataset_name):
            if dataset_name.endswith('.json'):
                ...
                train_data = []
                eval_data = []
                for item in original_data["train"]:
                    if item['question'] not in train_set:
                        train_data.append(item)
                        train_set.add(item['question'])
                for item in original_data["test"]:
                    if item['question'] not in eval_set:
                        eval_data.append(item)
                        eval_set.add(item['question'])
...
# English dataset
class BaichuanKBDataset(PromptRawDataset):

    def __init__(self, output_path, seed, local_rank, dataset_name):
        super().__init__(output_path, seed, local_rank, dataset_name)
        self.dataset_name = "baichuan2-7b-chat-KB-train"
        self.dataset_name_clean = "baichuan2-7b-chat-KB-train"
...
class KBDataset_for_ppo(PromptRawDataset):
    def __init__(self, output_path, seed, local_rank, dataset_name):
        super().__init__(output_path, seed, local_rank, dataset_name)
        self.dataset_name = "KBDataset_for_ppo"
        self.dataset_name_clean = "KBDataset_for_ppo"
    ...
    def get_prompt(self, sample):
        return self.B_INST + ' ' + sample['question'] + ' ' + self.E_INST + ' '

    def get_chosen(self, sample):
        return sample['positive_answer']

    def get_rejected(self, sample):
        return sample['negative_answer']

    def get_prompt_and_chosen(self, sample):
        return self.B_INST + ' ' + sample['question'] + ' ' + self.E_INST + ' ' + sample['positive_answer']

    def get_prompt_and_rejected(self, sample):
        return self.B_INST + ' ' + sample['question'] + ' ' + self.E_INST + ' ' + sample['negative_answer']
